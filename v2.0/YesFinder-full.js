    ...
    YesFinder.prototype = {
        init: function(){
            ...
            if(options.modal){
                ...
                this.YesModal.title('YesFinder - Web File Manager');
                this.YesModal.body($finder);
        ...
        load: function(option){
            ...
            switch(option){
                ...
                case 'upload':
                    ...
                    this.modal.button('okay').text('+ 添加文件').click(function(){input.click();});
                    this.modal.button('cancel').text('上 传').click(function(){
                        //that.modal.hide();
                    ...
                    this.upfiles = [];//清空上传文件
                    //以下为弹窗
                    this.modal.title('文件上传');
                    this.modal.body(body).css({padding:'5px 10px'});
        ...
        newfolder:function(path){
            ...
            input.before('<br><br><label>新目录：</label> ').after('<p class="red"></p>');
            this.modal.init(500,110);
            this.modal.title('新建目录');
            this.modal.body(body);
            this.modal.button('okay').click(function(){
                var name = input.val();
                if(!name||!name.length){
        ...
        delete: function(file){
            ...
            that.confirm('确认要删除此文件或目录: '+file+' ?',function(){
        ...
        alert: function(msg){
            ...
            this.modal.body($('<div>').css('padding','10px').html(msg));
            this.modal.button('okay').hide();
            this.modal.button('cancel').text('确认');
            this.modal.show();
            return false;
        },
        confirm: function(msg,okAction,cancelAction){
            ...
            this.modal.body($('<div>').css('padding','10px').html(msg));
            this.modal.button('okay').click(okAction);
            if(cancelAction)this.modal.button('cancel').click(cancelAction);
            this.modal.show();
        }
    };
...
(function($, window, document,undefined) {
    ...
    var YesModal = function(ele,options) {
        this.$ele = ele;
        this.options = $.extend({},{width:600,height:300},options);
    ...
    YesModal.prototype = {
        init: function(w,h){
            var that = this;
            this.dialog.html('<div class="modal-header"><b></b><a class="close" title="close">&times;</a></div>' +
                '<div class="modal-body"></div>' +
                '<div class="modal-footer">' +
                '<button name="okay">确认</button>' +
                '<button name="cancel">取消</button></div>');
            this.dialog.find('.close').click(function(){that.hide();});
            this.button('cancel').click(function(){that.hide();});
            if(this.options.title)this.title(this.options.title);
            if(this.options.body)this.body(this.options.body);
            if(this.options.footer)this.footer(this.options.footer);
            this.resize(w,h);
        ...
        resize: function(w,h){
            ...
            if(H>0) {
                //this.cover.css({width: W, height: H});
        ...
        title: function(value){
            var obj  = this.dialog.children('.modal-header');
            if(value)obj.children('b').html(value);
            return obj;
        },
        body: function(value){
            var obj  = this.dialog.children('.modal-body');
            if(value)obj.empty().append(value);
            return obj;
        },
        footer: function(value){
            var obj = this.dialog.children('.modal-footer');
            if(value)obj.empty().append(value);
            return obj;
        },
        button: function(name){
            return this.footer().children('button[name='+name+']');
        },
        show: function (){
...
(function($, window, document,undefined) {
    ...
    $.fn.YesContextMenu = function(data,options){
        ...
        data  = data ||[];
        options = $.extend(defaults,options||{});
        var idKey = 'menu_'+ new Date().getTime();
        $menu = $('<div id="'+idKey+'" class="contextmenu">');
        ...
        this.on('contextmenu',function(e){
            ...
            $menu.css({left: e.offsetX,top: e.offsetY});
            $menu.appendTo($(this)).show();
            return false;
        }).mouseleave(function(){
            $menu.fadeOut();
        });
    ...
    $.YesContextMenu = function(ele,data,options){
        return ele.ContextMenu(data,options);
    };
})(jQuery, window, document);
